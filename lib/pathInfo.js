...
module.exports = function(ops) {
  ...
  self.loadPackage = function(pkgPath) {
    if(cachedPackages[pkgPath]) return cachedPackages[pkgPath];

    var pkg = cachedPackages[pkgPath] = JSON.parse(fs.readFileSync(pkgPath, "utf8"));
  ...
  self.getPathInfo = function(required, cwd) {
    ...
    try {
      ...
      if(isModule && exists(coreModulePath)) {
        ...
        ret.path        = im ? browserifyPath(pkgPath) || realPath : realPath;
